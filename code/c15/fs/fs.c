...
static uint32_t get_parent_dir_inode_nr(uint32_t child_inode_nr,void* io_buf){
    ...
    inode_close(child_dir_inode);//取出来第二个目录项..所在的扇区就可以关闭了
    ide_read(cur_part->my_disk, block_lba, io_buf, 1);
    struct dir_entry* dir_e = (struct dir_entry*)io_buf;
    ASSERT(dir_e[1].i_no<4096 && dir_e[1].f_type == FT_DIRECTORY);
    return dir_e[1].i_no;   //..,即父目录的inode编号
}

/*在inode编号为p_inode_nr的目录中查找inode编号为c_inode_nr的子目录的名字, 将名族存入缓冲区
成功返回0, 失败返回-1*/
static int get_child_dir_name(uint32_t p_inode_nr, uint32_t c_inode_nr, char* path, void* io_buf){
    ...
    while(block_idx < block_cnt){
        if(all_blocks[block_idx]!=0){
            ...
            uint8_t dir_e_idx = 0;
            while(dir_e_idx < dir_entries_per_sec){
...
/*把当前工作目录的绝对路径写入buf, size是buf的大小, 失败返回NULL*/
char* sys_getcwd(char* buf, uint32_t size){

    ASSERT(buf != NULL);    //系统调用getcwd中要分配内存
    ...
    memset(buf, 0, size);
    char full_path_reverse[MAX_FILE_NAME_LEN];
    memset(full_path_reverse, 0, MAX_FILE_NAME_LEN);
    
    while((child_inode_nr)){ //还没到根目录
    ...
    //现在需要将full_path_reverse中的路径反置,以获得cwd
    char* last_slash;
